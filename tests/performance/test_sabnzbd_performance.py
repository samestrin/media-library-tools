...
class SABnzbdPerformanceTest(unittest.TestCase):
    ...
    def create_sabnzbd_directory(self, path: Path, include_nzb=True, include_par2=True, 
                                include_rar=True, include_admin=False) -> None:
    ...
    def measure_analysis_performance(self, directories: List[Path], runs=5) -> Dict[str, Any]:
        """
        ...
        for run in range(runs):
            ...
            for directory in directories:
                is_sabnzbd, score, indicators = self.detector.analyze_directory(directory)
    ...
    def test_baseline_performance_small(self):
        ...
        self.assertTrue(metrics['consistent_results'], "Results should be consistent across runs")
        self.assertLess(metrics['time_per_directory'], 0.1, "Should analyze each directory in <0.1s")
    ...
    def test_baseline_performance_medium(self):
        ...
        self.assertTrue(metrics['consistent_results'], "Results should be consistent across runs")
        self.assertLess(metrics['time_per_directory'], 0.05, "Should analyze each directory in <0.05s")
    ...
    def test_baseline_performance_large(self):
        ...
        metrics = self.measure_analysis_performance(directories, runs=3)  # Fewer runs for large dataset
        ...
        self.assertTrue(metrics['consistent_results'], "Results should be consistent across runs")
        self.assertLess(metrics['avg_time'], 300, "Should complete large dataset in <5 minutes")
    ...
    def test_complexity_analysis(self):
        ...
        for size in sizes:
            ...
            print(f"  {size} dirs: {metrics['avg_time']:.4f}s ({metrics['time_per_directory']:.6f}s per dir)")
        ...
        for i, size in enumerate(sizes[1:], 1):
            ...
            if complexity_factor > 1.5:
                print(f"    ⚠️  Possible non-linear complexity (factor {complexity_factor:.2f})")
            else:
    ...
    def test_memory_usage_baseline(self):
        ...
        print(f"Memory usage for 1000 directories:")
        ...
        self.assertLess(peak / 1024 / 1024, 100, "Peak memory should be <100MB for 1000 directories")
        self.assertLess(peak / len(directories), 10000, "Should use <10KB per directory")
