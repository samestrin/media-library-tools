...
def main():
    ...
    for size in test_sizes:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(
                f"      📈 Time per directory: {result['time_per_directory']:.6f} seconds"
            )
            print(
                f"      🚀 Directories per second: {result['directories_per_second']:.1f}"
            )
    ...
    if large_dataset_rate > 1000:
        print(
            f"✅ Large dataset performance: {large_dataset_rate:.1f} dirs/sec (Target: >1000)"
        )
    else:
        print(
            f"❌ Large dataset performance: {large_dataset_rate:.1f} dirs/sec (Target: >1000)"
    ...
    if medium_dataset_time < 0.001:
        print(
            f"✅ Medium dataset efficiency: {medium_dataset_time:.6f}s per dir (Target: <0.001s)"
        )
    else:
        print(
            f"❌ Medium dataset efficiency: {medium_dataset_time:.6f}s per dir (Target: <0.001s)"
    ...
    if detection_consistency < 0.1:
        print(
            f"✅ Consistent detection across dataset sizes (variance: {detection_consistency:.3f})"
        )
    else:
