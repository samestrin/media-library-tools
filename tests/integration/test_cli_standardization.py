...
class TestCLIStandardization(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.scripts = {
            ...
            # Remaining Scripts
            'plex_move_movie_extras': cls.project_root / 'plex' / 'plex_move_movie_extras',
            'plex_movie_subdir_renamer': cls.project_root / 'plex' / 'plex_movie_subdir_renamer',
            'plex_make_all_seasons': cls.project_root / 'plex' / 'plex_make_all_seasons',
        }
    ...
    def _run_script_with_args(self, script_path: Path, args: List[str], 
        ...
        for name, script_path in self.scripts.items():
            with self.subTest(script=name):
                ...
                self.assertIn('--dry-run', help_text,
                    f"Script {name} missing --dry-run argument")
        ...
        for name, script_path in self.scripts.items():
            with self.subTest(script=name):
                ...
                self.assertIn('--verbose', help_text,
                    f"Script {name} missing --verbose argument")
                
                # Check for --debug flag
                self.assertIn('--debug', help_text,
                    f"Script {name} missing --debug argument")
            ...
            with self.subTest(script=name):
                ...
                for arg in standard_args:
                    self.assertIn(arg, help_text,
                        f"Script {name} missing standard argument {arg}")
        ...
        for name, script_path in self.scripts.items():
            with self.subTest(script=name):
                ...
                self.assertIn('Examples:', help_text,
                    f"Script {name} missing Examples section")
    ...
    def test_version_flag_consistency(self):
        ...
        for name, script_path in self.scripts.items():
            with self.subTest(script=name):
                result = self._run_script_with_args(script_path, ['--version'], timeout=10)
    ...
    def test_special_script_requirements(self):
        ...
        self.assertNotEqual(result.returncode, 0,
            "plex_move_movie_extras should require arguments")
...
class TestGlobalConfigCLIIntegration(unittest.TestCase):
    ...
    def test_global_config_integration(self):
        ...
        self.assertEqual(result.returncode, 0,
            "Script should work with global configuration present")
