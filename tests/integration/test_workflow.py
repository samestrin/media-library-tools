...
class TestSABnzbdToPlexWorkflow(MediaLibraryTestCase):
    ...
    def test_movie_download_to_plex_workflow(self):
        ...
        organized_movies = list(plex_movies_dir.glob("**/*.mp4")) + list(
            plex_movies_dir.glob("**/*.mkv")
        )
        self.assertGreater(
            len(organized_movies), 0, "Should have organized movie files"
        )
    ...
    def test_tv_show_download_to_plex_workflow(self):
        ...
        self.assertGreater(
            len(episodes_before), 0, "Should have episode files to organize"
        )
    ...
    def test_mixed_media_workflow(self):
        ...
        if not (test_dir / "mixed_content").exists():
            ...
            # Copy movie content
            movie_fixture = self.fixture_manager.copy_fixture_to_test_data(
                "plex/movies/movie_with_extras"
            )
            for file_path in movie_fixture.glob("*"):
                shutil.copy2(file_path, mixed_content_dir / file_path.name)

            # Copy TV content
            tv_fixture = self.fixture_manager.copy_fixture_to_test_data(
                "plex/tv_shows/unorganized_episodes"
            )
...
class TestPlexOrganizationWorkflow(MediaLibraryTestCase):
    ...
    def test_movie_extras_organization_workflow(self):
        ...
        video_files = [
            f for f in all_files if f.suffix.lower() in [".mp4", ".mkv", ".avi"]
        ]
        ...
        extras_files = [
            ...
            if "trailer" in f.name.lower()
            or "behind" in f.name.lower()
            or "deleted" in f.name.lower()
        ]
    ...
    def test_batch_season_organization_workflow(self):
        ...
        self.assertGreater(
            len(found_seasons), 1, "Should have multiple seasons across shows"
        )
...
class TestPerformanceWorkflow(MediaLibraryTestCase):
    ...
    def test_deep_directory_structure_workflow(self):
        ...
        try:
            ...
            self.fail(
                "Should handle deep directory structures without recursion errors"
            )
