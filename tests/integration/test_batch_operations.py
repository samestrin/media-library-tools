...
class TestBatchSABnzbdOperations(MediaLibraryTestCase):
    ...
    def test_batch_directory_analysis(self):
        ...
        for dir_path, is_sabnzbd, score, indicators in sabnzbd_results:
            ...
            self.assertGreater(len(indicators), 0, f"Should have indicators for {dir_path}")
    ...
    def test_batch_size_calculation(self):
        ...
        from sabnzbd_cleanup import get_dir_size
        ...
        for i, (expected, calculated) in enumerate(zip(expected_sizes, calculated_sizes)):
            self.assertGreaterEqual(calculated, expected, 
...
class TestBatchPlexMovieOperations(MediaLibraryTestCase):
    ...
    def test_batch_movie_renaming(self):
        ...
        for i, movie_file in enumerate(movie_files):
            ...
            # Create extras files
            for j, extra_file in enumerate(extras_files):
        ...
        self.assertGreater(len(video_files), expected_total, 
                        "Should recognize all video files")
        
        # Separate main movies from extras
        main_movies = [f for f in video_files if any(movie in f.name for movie in movie_files)]
        extras = [f for f in video_files if f not in main_movies]
        
        self.assertGreater(len(main_movies), len(movie_files), "Should identify main movies")
    ...
    def test_batch_directory_creation(self):
        ...
        video_files = [f for f in processable_files 
...
class TestBatchTVShowOperations(MediaLibraryTestCase):
    ...
    def test_batch_season_organization(self):
        ...
        self.assertEqual(len(video_episodes), len(all_episodes), 
        ...
        self.assertGreater(len(show_seasons), 3, "Should have multiple show/season combinations")
    ...
    def test_batch_all_seasons_organization(self):
        ...
        shows_data = {
            'Show1': {
                'seasons': [1, 2, 3],
                'episodes_per_season': [3, 4, 2]
            },
            'Show2': {
                'seasons': [1, 2],
                'episodes_per_season': [5, 3]
            },
            'Show3': {
                'seasons': [1],
        ...
        for show_name, show_info in shows_data.items():
            ...
            for season_num, episode_count in zip(show_info['seasons'], show_info['episodes_per_season']):
        ...
        recognized_episodes = [ep for ep in all_episodes if batch_organizer.is_video_file(ep)]
        self.assertEqual(len(recognized_episodes), len(all_episodes), 
                        "Should recognize all episodes")
        
        # Calculate expected totals
        total_expected_episodes = sum(
            sum(show_info['episodes_per_season']) 
            for show_info in shows_data.values()
        )
        
        self.assertEqual(len(all_episodes), total_expected_episodes, 
        ...
        self.assertEqual(len(shows_with_episodes), len(shows_data), 
                        "Should have episodes for all shows")
        
        # Verify each movie has the expected number of extras
        for show_name, show_info in shows_data.items():
            show_dir = batch_all_seasons_dir / show_name
            show_extras = [f for f in show_dir.glob('*.*') 
                          if f.name != f"{show_name}.mkv"]
            
            # Note: This test is checking for episodes, not extras, so we'll verify episodes
            show_episodes = [f for f in show_dir.glob('*.*') 
                            if f.name.startswith(show_name)]
            
            self.assertGreater(len(show_episodes), 0, 
                           f"Show {show_name} should have episodes")
...
class TestBatchMovieExtrasOperations(MediaLibraryTestCase):
    ...
    def test_batch_extras_organization(self):
        ...
        recognized_extras = [extra for extra in all_extras 
                           if extras_organizer.is_video_file(extra.name)]
        
        self.assertEqual(len(recognized_extras), len(all_extras), 
        ...
        self.assertEqual(len(movies_with_extras), len(movies), 
                        "Should have extras for all movies")
        
        # Verify each movie has the expected number of extras
        for movie in movies:
            movie_dir = batch_extras_dir / movie
            movie_extras = [f for f in movie_dir.glob('*.*') 
                          if f.name != f"{movie}.mkv"]
            
            self.assertEqual(len(movie_extras), len(extras_types), 
...
class TestBatchPerformanceOperations(MediaLibraryTestCase):
    ...
    def test_large_batch_processing(self):
        ...
        self.assertEqual(len(processed_files), num_files, 
        ...
        print(f"Processing rate: {len(processed_files)/processing_time:.1f} files/second")
    
    @unittest.skipIf(PlexMovieSubdirRenamer is None or PlexDirectoryCreator is None or SeasonOrganizer is None or BatchSeasonOrganizer is None or not TEST_HELPERS_AVAILABLE, "Required modules not available")
        ...
        for tool in tools:
            ...
            for file_path in test_files:
                try:
                    ...
                    self.fail(f"Tool {tool.__class__.__name__} failed on concurrent access: {e}")
            
            results.append(tool_results)
        
        # Verify all tools processed all files
        for i, tool_results in enumerate(results):
            self.assertEqual(len(tool_results), len(test_files), 
                           f"Tool {i} should process all files")
