...
class TestFixtureExamples(MediaLibraryTestCase):
    ...
    def test_plex_tv_unorganized_episodes_fixture(self):
        ...
        s01_episodes = [f for f in self.get_file_list(test_dir, '*S01E*')]
        s02_episodes = [f for f in self.get_file_list(test_dir, '*S02E*')]
        s03_episodes = [f for f in self.get_file_list(test_dir, '*S03E*')]
    ...
    def test_plex_api_fixtures(self):
        ...
        import json
        with open(server_dir / 'server_info.json', 'r') as f:
