...
@unittest.skipIf(
    SABnzbdDetector is None or not TEST_HELPERS_AVAILABLE, "SABnzbd tools not available"
)
...
class TestPlexToolsErrorHandling(MediaLibraryTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.renamer = (
            PlexMovieSubdirRenamer() if PlexMovieSubdirRenamer is not None else None
        )
        self.creator = (
            PlexDirectoryCreator() if PlexDirectoryCreator is not None else None
        )
        self.season_organizer = (
            SeasonOrganizer() if SeasonOrganizer is not None else None
        )
        self.batch_organizer = (
            BatchSeasonOrganizer() if BatchSeasonOrganizer is not None else None
        )
        self.extras_organizer = (
            PlexMovieExtrasOrganizer() if PlexMovieExtrasOrganizer is not None else None
        )
    ...
    def test_special_characters_in_filenames(self):
        ...
        for test_file in created_files:
            ...
            try:
                ...
                self.fail(
                    f"Tools should handle special characters in '{test_file}': {e}"
                )
...
class TestResourceExhaustionScenarios(MediaLibraryTestCase):
    ...
    def test_deep_recursion_limits(self):
        ...
        try:
            ...
            self.fail(
                "Tools should handle deep directory structures without recursion errors"
            )
        except Exception:
            # Other exceptions might be acceptable
            pass
        ...
        for malformed_path in malformed_paths:
            ...
            try:
                ...
                if malformed_path.suffix.lower() == ".mp4":
                    self.assertTrue(
                        result, f"Failed for malformed path: {malformed_path}"
                    )
            except Exception as e:
