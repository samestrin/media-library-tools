...
def load_plex_server_episode_refresh():
    """Load the plex_server_episode_refresh script as a module."""
    script_path = (
        Path(__file__).parent.parent / "plex-api" / "plex_server_episode_refresh"
    )
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "plex_server_episode_refresh", temp_script_path
        )
...
class TestCredentialHandling(MediaLibraryTestCase):
    ...
    def test_get_token_from_env(self):
        ...
        client = self.module.PlexServerEpisodeRefresh(
            token=None, server_url="http://test:32400", verbose=False, debug=False
        )
    ...
    def test_get_server_from_env(self):
        ...
        client = self.module.PlexServerEpisodeRefresh(
            token="test_token", server_url=None, verbose=False, debug=False
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with patch("pathlib.Path.home", return_value=Path(temp_dir)):
                    ...
                    # ENV should win over files
                    with patch.dict(
                        os.environ, {"PLEX_SERVER": "http://env-server:32400"}
                    ):
                        client = self.module.PlexServerEpisodeRefresh(
                            token="test_token",
                            server_url=None,
...
class TestPlexAPIClient(MediaLibraryTestCase):
    ...
    def test_make_request_put_method(self):
        ...
        with patch("urllib.request.urlopen", mock_urlopen):
            result = self.client._make_request(
                "http://test-server:32400/test", method="PUT"
            )
    ...
    def test_make_request_with_data(self):
        ...
        with patch("urllib.request.urlopen", mock_urlopen):
            result = self.client._make_request(
                "http://test-server:32400/test", data=test_data
            )
...
class TestErrorHandling(MediaLibraryTestCase):
    ...
    def test_refresh_episode_image_empty_metadata(self):
        ...
        empty_metadata = {"MediaContainer": {"Metadata": []}}
    ...
    def test_complete_episode_refresh_workflow(self):
        ...
        with patch.object(client, "_make_request", side_effect=mock_make_request):
            ...
            expected_calls = [
                ...
                (
                    "http://test:32400/library/metadata/12345",
                    "GET",
                ),  # Get metadata again
                (
                    "http://test:32400/library/metadata/12345/posters",
                    "DELETE",
                ),  # Clear thumbnails
                (
                    "http://test:32400/library/metadata/12345/analyze",
                    "PUT",
                ),  # Re-analyze
            ]
    ...
    def test_credential_integration_with_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                client = self.module.PlexServerEpisodeRefresh(
                    token=None, server_url=None, verbose=False, debug=False
                )
