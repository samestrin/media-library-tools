...
try:
    ...
    spec1 = importlib.util.spec_from_file_location(
        "plex_make_seasons", seasons_script_path
    )
    ...
    spec2 = importlib.util.spec_from_file_location(
        "plex_make_all_seasons", all_seasons_script_path
    )
    ...
    def test_extended_season_patterns_consistency(self):
        ...
        for filename in test_cases:
            with self.subTest(filename=filename):
                ...
                all_seasons_result = self.all_seasons_organizer.extract_season_episode(
                    filename
                )
                ...
                self.assertIsNotNone(
                    seasons_season,
                    f"plex_make_seasons should detect season in '{filename}'",
                )
                self.assertIsNotNone(
                    all_seasons_season,
                    f"plex_make_all_seasons should detect season in '{filename}'",
                )
    ...
    def test_false_positive_consistency(self):
        ...
        for filename in false_positive_cases:
            with self.subTest(filename=filename):
                ...
                self.assertIsNone(
                    seasons_season,
                    f"plex_make_seasons should reject false positive '{filename}'",
                )
                self.assertIsNone(
                    all_seasons_season,
                    f"plex_make_all_seasons should reject false positive '{filename}'",
                )
