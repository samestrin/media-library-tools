...
PlexMovieSubdirRenamer = (
    getattr(plex_movie_subdir_renamer, "PlexMovieSubdirRenamer", None)
    if plex_movie_subdir_renamer
    else None
)
categorize_file = (
    getattr(plex_movie_subdir_renamer, "categorize_file", None)
    if plex_movie_subdir_renamer
    else None
)
clean_title = (
    getattr(plex_movie_subdir_renamer, "clean_title", None)
    if plex_movie_subdir_renamer
    else None
)
...
PlexDirectoryCreator = (
    getattr(plex_make_dirs, "PlexDirectoryCreator", None) if plex_make_dirs else None
)

plex_make_seasons = load_plex_tool("plex_make_seasons")
SeasonOrganizer = (
    getattr(plex_make_seasons, "SeasonOrganizer", None) if plex_make_seasons else None
)

plex_make_years = load_plex_tool("plex_make_years")
YearOrganizer = (
    getattr(plex_make_years, "YearOrganizer", None) if plex_make_years else None
)

plex_make_all_seasons = load_plex_tool("plex_make_all_seasons")
BatchSeasonOrganizer = (
    getattr(plex_make_all_seasons, "SeasonOrganizer", None)
    if plex_make_all_seasons
    else None
)

plex_move_movie_extras = load_plex_tool("plex_move_movie_extras")
PlexMovieExtrasOrganizer = (
    getattr(plex_move_movie_extras, "PlexMovieExtrasOrganizer", None)
    if plex_move_movie_extras
    else None
)
...
class TestPlexMovieSubdirRenamer(MediaLibraryTestCase):
    ...
    def test_is_video_file(self):
        ...
        video_structure = {"sample.mp4": None, "sample.mkv": None, "sample.avi": None}
        ...
        image_structure = {"poster.jpg": None, "fanart.png": None}
...
@unittest.skipIf(
    SeasonOrganizer is None or not TEST_HELPERS_AVAILABLE, "Plex tools not available"
)
...
@unittest.skipIf(
    YearOrganizer is None or not TEST_HELPERS_AVAILABLE, "Plex tools not available"
)
class TestYearOrganizer(MediaLibraryTestCase):
    ...
    def test_extract_year_info(self):
        ...
        year, pattern_desc, matched_text = self.organizer.extract_year_info(
            "Movie.Title.(2023)"
        )
...
class TestPlexMovieExtrasOrganizer(MediaLibraryTestCase):
    ...
    def test_is_video_file(self):
        """Test is_video_file method."""
        # Use fixture files for testing
        self.copy_fixture("common/video_files")
        ...
        # Test with invalid extensions from fixtures
        self.copy_fixture("common/image_files")
