...
class TestCredentialHandling(MediaLibraryTestCase):
    ...
    def test_get_tvdb_key_from_cli(self):
        ...
        result = self.module.get_tvdb_key_from_sources(cli_key="test_key_cli", debug=False)
    ...
    def test_cli_priority_over_env(self):
        ...
        with patch.dict(os.environ, {'TVDB_API_KEY': 'env_key'}):
            result = self.module.get_tvdb_key_from_sources(cli_key="cli_key", debug=False)
    ...
    def test_get_tvdb_key_from_global_env_file(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch('os.path.exists', side_effect=lambda path: path != '.env'):
                with patch('pathlib.Path.home') as mock_home:
                    mock_home.return_value = Path('/home/user')
                    with patch('builtins.open', mock_open(read_data=global_env_content)):
                ...
                with patch('pathlib.Path.home') as mock_home:
                    ...
                    with patch.object(Path, 'exists', return_value=True):
                        ...
                        def mock_open_func(file_path, *args, **kwargs):
                            if str(file_path).endswith('.env') and not str(file_path).startswith('/home'):
...
class TestTVDBClient(MediaLibraryTestCase):
    ...
    def test_search_show_success(self, mock_urlopen):
        ...
        mock_urlopen.return_value.__enter__.side_effect = [login_response, search_response]
...
class TestTVShowYearUpdater(MediaLibraryTestCase):
    ...
    def test_needs_year_update(self):
        ...
        needs_update, current_year = self.updater.needs_year_update("Show Title (2019)", 2020)
        self.assertTrue(needs_update)
        self.assertEqual(current_year, 2019)
        
        # Correct year, no update needed
        needs_update, current_year = self.updater.needs_year_update("Show Title (2020)", 2020)
    ...
    def test_get_credential_from_sources_priority_order(self):
        ...
        result = self.updater._get_credential_from_sources("TEST_CREDENTIAL", "cli_value")
...
class TestIntegrationWorkflow(MediaLibraryTestCase):
    ...
    def test_dry_run_workflow(self, mock_urlopen):
        ...
        mock_urlopen.return_value.__enter__.side_effect = [
            login_response, search_responses[0], search_responses[1]
        ]
