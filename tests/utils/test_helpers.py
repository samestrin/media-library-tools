...
class MediaLibraryTestCase(unittest.TestCase):
    ...
    def assert_file_count(self, directory: Union[str, Path], expected_count: int, 
                         pattern: str = '*', msg: Optional[str] = None):
        ...
        if actual_count != expected_count:
            self.fail(msg or f"Expected {expected_count} files matching '{pattern}', "
    ...
    def get_dir_list(self, directory: Union[str, Path], pattern: str = '*') -> List[Path]:
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            with patch('builtins.input', return_value='y'):
                with patch('sys.stdin.isatty', return_value=not return_value):
...
def create_plex_movie_fixture(base_dir: Path, with_extras: bool = True) -> Path:
    ...
    if with_extras:
        ...
        manager.create_video_files(featurettes_dir, [
        ...
        manager.create_video_files(extras_dir, [
            'trailer.mp4',
            'teaser.mkv'
        ])
...
def create_plex_tv_fixture(base_dir: Path, num_seasons: int = 2, episodes_per_season: int = 5) -> Path:
