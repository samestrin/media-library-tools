...
class TestRunner:
    ...
    def discover_tests(self, pattern: str = None, subdir: str = None) -> List[str]:
        ...
        if subdir:
            ...
            for subdir_path in self.tests_dir.iterdir():
                if (
                    subdir_path.is_dir()
                    and not subdir_path.name.startswith(".")
                    and subdir_path.name not in test_subdirs
                ):
                    for test_file in subdir_path.glob(pattern):
                        if test_file.is_file():
                            test_files.append(str(test_file))
    ...
    def run_test_file(self, test_file: str, timeout: int = None) -> TestResult:
        ...
        try:
            ...
            process = subprocess.run(
                cmd, capture_output=True, text=True, timeout=timeout, cwd=self.tests_dir
            )
    ...
    def _parse_unittest_output(
        self, result: TestResult, stdout: str, stderr: str
    ) -> None:
        """
    ...
    def run_all_tests(self) -> None:
        ...
        if self.args.categories:
            ...
            for category in TEST_CATEGORIES:
                if (
                    not self.args.skip_categories
                    or category not in self.args.skip_categories
                ):
                    category_results = self.run_category(category)
                    self.results.extend(category_results)
    ...
    def save_report(self) -> None:
        ...
        try:
            with open(output_path, "w") as f:
                f.write("Media Library Tools - Test Report\n")
...
""",
    ...
    parser.add_argument(
        "--pattern", help='File pattern to match (e.g., "test_sabnzbd_*.py")'
    )
    ...
    # Output options
    parser.add_argument("--output-file", "-o", help="Save test report to file")
    parser.add_argument(
        "--show-performance", action="store_true", help="Show performance summary"
    )
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Minimize output (only show summary)"
    )
    ...
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate test environment"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.coverage:
            ...
            if args.cov_html or args.coverage:
                subprocess.run(
                    [sys.executable, "-m", "coverage", "html", "-d", str(cov_html_dir)],
                    check=False,
                )
            if args.cov_xml or args.coverage:
                subprocess.run(
                    [sys.executable, "-m", "coverage", "xml", "-o", str(cov_xml_file)],
                    check=False,
                )
